...
const endpointFile=path.join(root,process.env.CODEX_WALLPAPERS_ENDPOINT || 'router-endpoint.json');
...
async function verifyOwner(endpoint){
  ...
  await exec('powershell.exe',['-NoProfile','-ExecutionPolicy','Bypass','-File',script,'-VerifyEndpoint',endpointFile],{windowsHide:true,timeout:20000});
  const verified=JSON.parse((await fs.readFile(endpointFile,'utf8')).replace(/^\uFEFF/,''));
