...
const image=await importMedia(png,{root:data,title:'Test image <script>'});const movie=await importMedia(video,{root:data,title:'Test video'});
...
try{
 ...
 assert.equal(await page.locator('textarea').count(),1);await page.locator('textarea').fill('The composer still works.');results.push('Image; palette; rounded settings; composer input preserved');
